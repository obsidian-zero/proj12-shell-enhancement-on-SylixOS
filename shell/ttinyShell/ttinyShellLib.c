...
static PLW_LIST_LINE    _G_plineTSKeyHeaderHashTbl[LW_CFG_SHELL_KEY_HASH_SIZE];
                                                                        /*  哈希搜索表                  */
BOOL   AUTO_CMPL = FALSE;
INT    SELECTED_INDEX = 0;
INT    MATECHED_KEYWORD_NUM = 0;
__PTSHELL_KEYWORD   ppskwNode[__TTINY_SHELL_KEYWORD_BUFFER];
/*********************************************************************************************************
  强制使用内建命令表
*********************************************************************************************************/
#if LW_CFG_MODULELOADER_EN > 0
...
#define __TTINY_SHELL_CMD_ISEND(pcCmd)      (*(pcCmd) == PX_EOS)
    ...
    while(TRUE) {
        ...
        else if (cKeyword[i + 1] == 0 && cKeyword[i] == '?' && i != 0) {
            checkcKeyword = TRUE;
            break;
        }
        i++;
    ...
    if (checkcKeyword && !pcParam) {
        __TTINY_SHELL_LOCK();

        SELECTED_INDEX = 0;
        MATECHED_KEYWORD_NUM = 0;

        i = 0;
        while(i < __TTINY_SHELL_KEYWORD_BUFFER) {
            ppskwNode[i] = LW_NULL;
            i++;
        }
        ...
        while(plineNode != LW_NULL) {
            pskwNode = _LIST_ENTRY(plineNode, __TSHELL_KEYWORD,
                                   SK_lineManage);

            plineNode = _list_line_get_next(plineNode);

            if(lib_strlen(pskwNode->SK_pcKeyword) < lib_strlen(cKeyword) - 1)
                continue;

            for (j = 0; j < lib_strlen(cKeyword) - 1; j++) {
                if ((pskwNode->SK_pcKeyword)[j] != cKeyword[j]) {
                    break;
                }
            }

            if(j != lib_strlen(cKeyword) - 1)
                continue;

            ppskwNode[MATECHED_KEYWORD_NUM++] = pskwNode;
        }

        __TTINY_SHELL_UNLOCK();

        if (!MATECHED_KEYWORD_NUM)
            return (-ERROR_TSHELL_AUTO_CMPL_FAILED);
     ...
     *  判断是否需要关键字、参数联想
     */
    if(lib_strcmp(pcParam,"?") == 0){                                 /*  判断参数是否为单个 '?'     */
        if(ERROR_NONE == __tshellKeywordFind(cKeyword, &pskwNode)){
            if(pskwNode->SK_pcFormatString){
                printf("%-20s", pskwNode->SK_pcKeyword);
                API_TShellColorStart2(LW_TSHELL_COLOR_GREEN, STD_OUT);
                printf("%s", pskwNode->SK_pcFormatString);            /*  打印参数信息                */
            ...
            MATECHED_KEYWORD_NUM = 0;
            i = 0;
            while(i < __TTINY_SHELL_KEYWORD_BUFFER) {                 /*  清空自动补全列表             */
                ppskwNode[i] = LW_NULL;
                i++;
            }
            ppskwNode[MATECHED_KEYWORD_NUM++] = pskwNode;             /*  将指令添加入自动补全列表     */
            AUTO_CMPL = TRUE;
            return  (ERROR_NONE);
        }
...
static PVOID  __tshellBackground (PVOID  pvArg)
{
    ...
    if ((iRetValue < 0) && !tsbg->TSBG_bIsJoin) {                       /*  出现错误且sh没有等待此线程  */
        ...
        case -ERROR_TSHELL_AUTO_CMPL_FAILED:
            fprintf(stderr, "sh: automatic completion failed, command not found.\n");
            break;
