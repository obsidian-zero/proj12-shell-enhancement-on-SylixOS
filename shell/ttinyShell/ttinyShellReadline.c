...
** 调用模块:
*********************************************************************************************************/
static VOID  __tshellAutoCmpl (INT  iFd, __PSHELL_INPUT_CTX  psicContext)
{
    INT i=0;
    while(i<lib_strlen(ppskwNode[SELECTED_INDEX]->SK_pcKeyword)){
        write(iFd, &ppskwNode[SELECTED_INDEX]->SK_pcKeyword[i], 1);
        CTX_BUFFER[CTX_CURSOR] = ppskwNode[SELECTED_INDEX]->SK_pcKeyword[i];
        CTX_CURSOR++;
        CTX_TOTAL++;
        i++;
    }
...
/*********************************************************************************************************
** 函数名称: __tshellAlterAutoCmpl
** 功能描述: 删除掉原本自动补全的指令，切换自动补全的指令index
** 输　入  : iFd                           文件描述符
**           psicContext                   当前输入上下文
** 输　出  : NONE
** 全局变量:
** 调用模块:
*********************************************************************************************************/
static VOID  __tshellAlterAutoCmpl (INT  iFd, __PSHELL_INPUT_CTX  psicContext)
{
#define __KEY_BS    0x08
    ...
    if(MATECHED_KEYWORD_NUM == SELECTED_INDEX) {
        SELECTED_INDEX = 0;
    }
}
/*********************************************************************************************************
...
#define __CHK_READ_OUT(res)     if (sstReadNum <= 0) {  \
    ...
    if (AUTO_CMPL) {
        __tshellAutoCmpl(iFd, &sicContext);                             /* 自动补全指令                 */
    }
...
__re_check_key:
        if (__KEY_IS_ESC(cRead)) {                                      /*  功能键起始                  */
            ...
            if (AUTO_CMPL) {
                __tshellAlterAutoCmpl(iFd, &sicContext);                /*  自动补全开启时。tab切换指令 */
                __tshellAutoCmpl(iFd, &sicContext);
            } else {
